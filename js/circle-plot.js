...
Assembly.prototype.drawPlot = function(parent_div, longest, circle_span) {
  ...
  if (this.busco) {
    ...
    key.append('rect').attr('height', w).style("border", "2px solid black").attr('width', w).attr('class', 'asm-busco_C asm-toggle');
    ...
    key.append('rect').attr('y', w * 4.5).attr('height', w).attr('width', w).attr('class', 0);
    key.append('text').attr('x', w + 4.5).attr('y', w * 4.5 - 1).text('Miss. (' + this.busco.M.toFixed(1) + '%)').attr('class', 'asm-key');
  }
  else if (this.cegma_complete) {
    var lccg = lg.append('g')
